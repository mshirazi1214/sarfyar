...
function LessonDisplay({ lessonId, highlightInfo }) {
  ...
  const sectionRefs = {
    ...
    examples: React.useRef(null),
    conjugationTable: React.useRef(null),
    vocabulary: React.useRef(null)
    // Add other keys here if your mockLessonContent sections have different keys
    // that you want to be able to scroll to, e.g., 'interactiveElements'.
  };
  ...
  React.useEffect(() => {
    if (lessonId) {
      ...
      if (content) {
        ...
        setError(`متأسفانه، محتوایی برای درس با شناسه "${lessonId}" یافت نشد.`);
      }
    } else {
      setLessonContent(null);
      setError('');
  ...
  React.useEffect(() => {
    if (highlightInfo && highlightInfo.key && lessonContent) {
      document.querySelectorAll('.highlighted-section').forEach(el => el.classList.remove('highlighted-section'));

      const refToHighlight = sectionRefs[highlightInfo.key];
      if (refToHighlight && refToHighlight.current) {
        const element = refToHighlight.current;

        element.scrollIntoView({ behavior: 'smooth', block: 'center', inline: 'nearest' });

        // Apply highlight to the specific section content
        ...
        console.warn("Could not find element for highlight key:", highlightInfo.key, "Available refs:", Object.keys(sectionRefs));
      }
    }
  }, [highlightInfo, lessonContent]); // Removed sectionRefs from dependencies as it's stable


  if (!lessonId) return null;
  if (error) return <div className="p-4 my-4 text-red-700 bg-red-100 border border-red-300 rounded-lg shadow-md">{error}</div>;
  if (!lessonContent) return <div className="p-4 my-4 text-orange-700 bg-orange-100 border border-orange-300 rounded-lg shadow-md">در حال بارگذاری محتوای درس...</div>;

  const { title, summary, examples, conjugationTable, vocabulary, interactiveElements } = lessonContent;

  // Assign an ID to the root div of LessonDisplay for App.js to scroll to if needed.
  return (
    <div id="lesson-display-component" className="lesson-display p-4 md:p-6 bg-white rounded-xl shadow-xl border border-gray-200">
      <h1 className="text-2xl md:text-3xl font-bold text-indigo-800 mb-6 border-b-2 border-indigo-100 pb-4">
        {title}
      </h1>

      {summary && (
        <section ref={sectionRefs.summary} className="mb-8 p-4 bg-gray-50 rounded-lg border border-gray-200 section-container">
          {/* Removed section-content-wrapper, ref is now on the section itself */}
      ...
      {examples && examples.length > 0 && (
        <section ref={sectionRefs.examples} className="mb-8 section-container">
           {/* Removed section-content-wrapper */}
      ...
      {conjugationTable && (
        <section ref={sectionRefs.conjugationTable} className="mb-8 section-container">
          {/* Removed section-content-wrapper */}
            <h2 className="text-xl font-semibold text-gray-700 mb-4 flex items-center">
            ...
            {conjugationTable.imagePlaceholder && ( <p className="text-xs text-gray-500 mt-2 italic"> (نمایش گرافیکی جدول از فایل: {conjugationTable.imagePlaceholder} در آینده امکان‌پذیر خواهد بود) </p> )}
        </section>
      )}

      {vocabulary && vocabulary.length > 0 && (
        <section ref={sectionRefs.vocabulary} className="mb-8 section-container">
          {/* Removed section-content-wrapper */}
            <h2 className="text-xl font-semibold text-gray-700 mb-4 flex items-center">
      ...
      {interactiveElements && (
        <section className="mt-10 pt-6 border-t-2 border-gray-100 section-container">
          {/* Removed section-content-wrapper */}
            ...
            {interactiveElements.highlightEnabled && ( <button className="px-4 py-2 bg-yellow-400 text-yellow-800 rounded-lg hover:bg-yellow-500 transition-colors text-sm font-medium shadow-sm hover:shadow-md disabled:opacity-60 disabled:cursor-not-allowed" disabled> نشانه‌گذاری متن (به‌زودی) </button> )}
            {interactiveElements.audioAvailable && ( <button className="px-4 py-2 bg-teal-400 text-teal-800 rounded-lg hover:bg-teal-500 transition-colors text-sm font-medium shadow-sm hover:shadow-md disabled:opacity-60 disabled:cursor-not-allowed" disabled> پخش صوتی خلاصه (به‌زودی) </button> )}
            {interactiveElements.downloadOptions && interactiveElements.downloadOptions.length > 0 && ( interactiveElements.downloadOptions.map(option => ( <button key={option} className="px-4 py-2 bg-purple-400 text-purple-800 rounded-lg hover:bg-purple-500 transition-colors text-sm font-medium shadow-sm hover:shadow-md disabled:opacity-60 disabled:cursor-not-allowed" disabled> دانلود {option} (به‌زودی) </button> )) )}
            </div>
        </section>
      )}
    </div>
